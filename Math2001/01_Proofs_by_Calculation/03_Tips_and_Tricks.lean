...
Exercise: choose some of these examples and type out the whole proofs
          printed in the text as Lean proofs. -/
...
Solve these problems yourself.  You may find it helpful to solve them on
paper before typing them up in Lean. -/
...
example {u v : ℝ} (h1 : u + 1 = v) : u ^ 2 + 3 * u + 1 = v ^ 2 + v - 1 :=
  calc u ^ 2 + 3 * u + 1 = (u + 1) ^ 2 + u := by ring
  _ = v^2 + u := by rw[h1]
  _ = v^2 + (u + 1) - 1 := by ring
  _ = v ^ 2 + v - 1 := by rw[h1]
...
example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 :=
  calc
    y = y + (2 - y) * (x + 3 - 3) / 2 - (2 * x - y * x) / 2 := by ring
    _ = y + (2 - y) * (5 - 3) / 2 - 0 / 2 := by rw[h1,h2]
    _ = 2 := by ring
